...
try:
    ...
    if not versions_match:
        ...
        os.system(f'pip install pandas=={required_pandas} numpy=={required_numpy} scikit-learn=={required_sklearn} plotly_express ydata-profiling openpyxl chardet --upgrade --quiet')
    ...
    os.system('pip install pandas==2.2.3 numpy==1.26.4 scikit-learn==1.6.1 plotly_express ydata-profiling openpyxl chardet --upgrade --quiet')
    st.experimental_rerun() # Rerun the app after installation
...
def detect_encoding(file_bytes):
    """
    Detects the encoding of the file using chardet library.
    Returns the best detected encoding.
    """
    try:
        # Sample first 100KB for encoding detection
        sample_size = min(100000, len(file_bytes))
        ...
        st.warning(f"Encoding detection failed: {e}. Using UTF-8 as fallback.")
        return 'utf-8'

def load_data_robustly(uploaded_file):
    """
    Enhanced automatic loading of CSV or Excel files with comprehensive encoding detection,
    multiple delimiter support, and robust error handling.
    """
    df = None
    ...
    if file_extension in ['.xls', '.xlsx', '.xlsm']:
        ...
        try:
            ...
            xls = pd.read_excel(io.BytesIO(file_bytes), sheet_name=None, decimal=",")
            sheet_names = list(xls.keys())
            st.info(f"📊 Available Sheets: {sheet_names}")
            
            # Use the first sheet by default
            if sheet_names:
                df = xls[sheet_names[0]]
                st.success(f"✅ Excel data successfully loaded from sheet: {sheet_names[0]}")
                st.info(f"📋 Sheet dimensions: {df.shape[0]} rows × {df.shape[1]} columns")
            else:
        ...
        st.info(f"📄 CSV file detected: {uploaded_file.name}")
        
        # Get file bytes for encoding detection
        file_bytes = uploaded_file.getvalue()
        ...
        encodings_to_try = [detected_encoding, 'utf-8', 'utf-16', 'cp1251', 'iso-8859-1', 'latin-1']
        
        # Remove duplicates while preserving order
        encodings_to_try = list(dict.fromkeys(encodings_to_try))
        
        # Multiple delimiters to try
        delimiters_to_try = [',', ';', '\t', '|', ' ']
        
        st.info(f"🔄 Trying encodings: {encodings_to_try}")
        st.info(f"🔄 Trying delimiters: {delimiters_to_try}")
        ...
        for encoding in encodings_to_try:
            ...
            try:
                ...
                st.info(f"📝 Successfully decoded with {encoding}")
                ...
                for delimiter in delimiters_to_try:
                    try:
                        ...
                        # Check if this is a reasonable DataFrame
                        if (len(df_attempt.columns) > 1 and 
                            df_attempt.shape[0] > 0 and
                            df_attempt.dropna(axis=1, how='all').shape[1] > 1):
                            
                            df = df_attempt
                            st.success(f"✅ Successfully loaded with encoding '{encoding}' and delimiter '{delimiter}' (header=0)")
                            st.info(f"📋 Data dimensions: {df.shape[0]} rows × {df.shape[1]} columns")
                            file_loaded = True
                            break
                            
                    except Exception as delimiter_error:
                        continue
                
                # If header=0 failed, try header=1
                if not file_loaded:
                    for delimiter in delimiters_to_try:
                        try:
                            df_attempt = pd.read_csv(
                            ...
                            if (len(df_attempt.columns) > 1 and 
                                ...
                                df = df_attempt
                                st.success(f"✅ Successfully loaded with encoding '{encoding}' and delimiter '{delimiter}' (header=1)")
                                st.info(f"📋 Data dimensions: {df.shape[0]} rows × {df.shape[1]} columns")
                                file_loaded = True
                                break
                ...
                st.warning(f"⚠️ Failed to decode with {encoding}")
                continue
            except Exception as e:
    ...
    if df is None:
        ...
        st.error("💡 Supported formats: CSV, Excel (.xls, .xlsx, .xlsm)")
        st.error("💡 Ensure your file has proper structure with headers and data")
    else:
        ...
        if null_counts > 0:
            st.info(f"📊 Found {null_counts} null values in the dataset")
        ...
        st.info(f"📝 Text columns: {len(text_cols)}")
...
st.header("⚙️ Step 2: Perform Analysis and Classification")
if st.session_state['original_df'] is not None:
