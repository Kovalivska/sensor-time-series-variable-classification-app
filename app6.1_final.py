...
st.set_page_config(layout="wide", page_title="Variable Classification App v6.1 (Enhanced File Loading)")
...
try:
    ...
    if not versions_match:
        ...
        os.system(f'pip install pandas=={required_pandas} numpy=={required_numpy} scikit-learn=={required_sklearn} plotly_express openpyxl chardet --upgrade --quiet')
    ...
    # Removed sweetviz from installation command as it's causing issues
    os.system('pip install pandas==2.2.3 numpy==1.26.4 scikit-learn==1.6.1 plotly_express openpyxl chardet --upgrade --quiet')
    st.experimental_rerun() # Rerun the app after installation

# --- Enhanced File Loading Functions ---

def detect_encoding(file_content, sample_size=10000):
    """
    Automatic file encoding detection.

    Args:
        file_content (bytes): File content in bytes
        sample_size (int): Sample size for encoding analysis
    ...
    try:
        ...
        # Return detected encoding if confidence is high
...
def load_data_robustly(uploaded_file):
    """
    Enhanced function for robust loading of CSV and Excel files.
    Automatically detects encoding, delimiters and file structure.
    ...
    try:
        ...
        if file_extension in ['.xls', '.xlsx', '.xlsm']:
            ...
            try:
                ...
                best_sheet = max(xls.values(), key=lambda x: x.shape[0] * x.shape[1])
                df = best_sheet
        ...
        # CSV file processing
            ...
            # Extended list of delimiters and encodings
            common_delimiters = [',', ';', '\t', '|', ' ']
            encodings_to_try = [detected_encoding, 'utf-16', 'utf-8', 'cp1251', 'iso-8859-1', 'latin-1']
            decimal_separators = [',', '.']
            ...
            for encoding in encodings_to_try:
                ...
                for delimiter in common_delimiters:
                    ...
                    for decimal_sep in decimal_separators:
                        if success:
                            break
                        ...
                        for header in header_options:
                            try:
                                ...
                                # Check data quality
                                if (len(df_attempt.columns) > 1 and
                                    df_attempt.shape[0] > 0 and
                                    df_attempt.dropna(axis=1, how='all').shape[1] >= 2):
                                    
                                    df = df_attempt
                                    st.success(f"✅ Successfully loaded with:")
                                    st.info(f"   - Encoding: {encoding}")
                                    st.info(f"   - Delimiter: '{delimiter}'")
                                    st.info(f"   - Decimal separator: '{decimal_sep}'")
                                    st.info(f"   - Header row: {header}")
                                    success = True
                                    break
            ...
            if not success:
                st.warning("❌ CSV file could not be loaded with standard parameters")
                
                # Final attempt using specific parameters
                try:
                    st.info("🔧 Attempting to load with UTF-16 parameters, comma as delimiter...")
                    df = pd.read_csv(
                        io.StringIO(file_content.decode('utf-16')),
                        sep=',',
                        decimal=',',
                        header=0,
                        low_memory=False
                    )
                    st.success("✅ Data successfully loaded with encoding='utf-16', sep=',', decimal=','")
                    success = True
    ...
    if df is not None and not df.empty:
        ...
        st.info(f"🧮 Size: {df.shape}")
        st.info(f"🏷️ Columns: {list(df.columns)}")
        ...
        if missing_data.sum() > 0:
            ...
            for col, missing_count in missing_summary.items():
                ...
                st.text(f"   {col}: {missing_count} ({percentage:.1f}%)")
        else:
        ...
        if len(categorical_cols) > 0:
            st.info(f"📝 Text/Categorical columns ({len(categorical_cols)}):")
        ...
        st.error("❌ Data loading failed. Check file path, format and encoding")
        return None
...
st.title("Variable Analysis and Classification v6.1 (Enhanced File Loading)")
...
st.markdown("**Enhanced with robust file loading:** Automatic encoding detection, multiple delimiter support, comprehensive error handling, and detailed loading statistics.")
st.markdown("---")
...
st.markdown("---")
st.info("Thank you for using the Variable Classification App v6.1!")
